...
export class RetentionEngine {
  ...
  async enhanceScript(projectId: string, script: string): Promise<string> {
    try {
      ...
      const analytics = await this.validation.validateAndSanitize(rawAnalytics);
      if (!analytics) return script; // dropped due to corruption

      // If retention is good, we do not inject anything.
      if (analytics.retention >= RetentionEngineService.RETENTION_THRESHOLD) {
        return script;
      }

      // Estimate how many interrupt markers we need based on script length.
      const wordCount = script.split(/\s+/).filter(Boolean).length;
      const estimatedSeconds = (wordCount / 100) * RetentionEngineService.SECONDS_PER_100_WORDS;
