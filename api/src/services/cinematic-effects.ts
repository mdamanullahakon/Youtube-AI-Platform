...
export function buildZoomFilter(zoomDirection: 'in' | 'out' | 'none', duration: number): string {
  const fps = 30;
  const totalFrames = Math.round(duration * fps);
  if (zoomDirection === 'in') {
    return `zoompan=z='if(lte(zoom,1.0),1.0,zoom-0.001)':d=${totalFrames}:s=1920x1080:fps=${fps}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'`;
  }
  if (zoomDirection === 'out') {
    return `zoompan=z='if(gte(zoom,1.3),1.3,zoom+0.002)':d=${totalFrames}:s=1920x1080:fps=${fps}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'`;
  }
  // 'none' — no zoom
  return `zoompan=z=1:d=${totalFrames}:s=1920x1080:fps=${fps}`;
}
